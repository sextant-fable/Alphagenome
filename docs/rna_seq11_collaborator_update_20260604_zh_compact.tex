...
\begin{frame}{旧 128bp 基线并没有饱和}
  ...
  \vspace{1.1em}
  \smallnote{Loss 做法：Improved 128bp model 在 full-log1p target 上训练 hybrid loss，即 $\alpha\cdot$MSE + $(1-\alpha)\cdot$SmoothL1；validation 指标仍用 unweighted full MSE/Pearson。}
...
\begin{frame}{Residual correction 是在 frozen 128bp prediction 上加一个小修正}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{实际做法}
        \begin{itemize}
          \item 先用 improved 128bp model 产生 full-resolution base prediction。
          \item 这个 128bp base head 和 AlphaGenome trunk 都保持 frozen。
          \item 另加一个 trainable 1bp residual head，只预测 correction update。
          \item 最终输出不是替换 base，而是 base + scaled 1bp update。
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.47\textwidth}
      \begin{block}{模型形式}
        \[
          \hat{y}
          =
          \mathrm{stopgrad}(\hat{y}_{128})
          +
          s_{\mathrm{track}} \cdot \Delta_{1bp}
        \]
        ...
        \begin{tabular}{ll}
          ...
          Term & Meaning \\
          \midrule
          $\hat{y}_{128}$ & frozen 128bp base prediction \\
          $\Delta_{1bp}$ & 1bp head predicted residual \\
          $s_{\mathrm{track}}$ & trainable per-track scale \\
          \bottomrule
        \end{tabular}
      \end{block}
    \end{column}
  ...
  \vspace{0.6em}
  \smallnote{直观上：128bp base 保留稳健的 broad RNA-seq prediction；1bp head 只负责补局部、高频或 high-signal 误差。}
\end{frame}
...
\begin{frame}{最好的模型已经进入一个很窄的 plateau}
  \begin{block}{Top15 的共同模式}
    \begin{itemize}
      ...
      \item Track-weighted loss 做法：训练时给指定 tracks 的每个有效位置乘更高权重；validation full metrics 仍保持 unweighted。
    \end{itemize}
  \end{block}
...
\begin{frame}{Signal-weighted loss 能改善 amplitude，但有代价}
  ...
  \vspace{0.8em}
  \smallnote{Loss 做法：在训练 batch 内按 true signal 选 high $>3$、top5\%、top1\% 或 intestine T1/T3/T4 high-signal 元素，把这些元素的基础 loss 乘上 preset factor，例如 top5x3 表示 top5 true-signal 元素权重为 $3\times$。}
...
\begin{frame}{Region-weighted loss 能改善 exon 和 gene-body 信号}
  ...
  \vspace{0.8em}
  \smallnote{Loss 做法：用 WBcel235 GTF 在每个训练窗口生成 exon、gene-body、promoter、intron、intergenic 位置权重，再把 per-base 基础 loss 乘上这些权重；v1 权重更强，v2 更温和。}
...
\begin{frame}{Gene auxiliary loss 和 calibration 还不是主方向}
  \begin{itemize}
    \item Gene auxiliary loss 做法：在主 per-base loss 外，加 $\lambda\cdot$MSE(predicted gene-body mean, true gene-body mean)，只统计 gene-body overlap 足够长的基因。
    ...
    \item Calibration 做法：冻结已有 prediction head 后只训练 per-track affine output scale/shift；它是输出校准，不是新的 weighted loss。
