...
## 2026-09-04 - P11-P25 Post-P10 Evidence Reconciliation

This append-only reconciliation was performed from the existing immutable specifications, execution records, result manifests, and phase reviews. It synchronizes this log with the current controller state; it does not change historical run timestamps, hashes, approvals, or scientific claims. P11 and P12 are the replicate-holdout continuation of the controlled v2 workflow. P13-P25 are post-completion evidence lanes and do not reopen P6C or alter the consumed final-test lock.

| Phase | Audited state | Evidence and bounded interpretation |
| --- | --- | --- |
| P11 | `R11 PASS` | The replicate-holdout data contract is complete: 482 source-run role rows, 241 training groups, 80 held-out groups, and 241 fold-specific track means. No locked-test signal was read. Evidence: `alphagenome_custom/metadata/v2/audits/P11/review.json` and `alphagenome_custom/metadata/v2/replicate_holdout_v1/`. |
| P12 | `R12 PASS` | The registered 165-job replicate-holdout matrix completed on physical GPUs 2/3, with 3,630 metric rows and 79,200 role-metric rows; locked-test access was prohibited and reads were zero. The failed first attempt remains preserved separately and is not counted. Evidence: `alphagenome_custom/metadata/v2/audits/P12/review.json` and `alphagenome_custom/metadata/v2/replicate_holdout_v1/`. |
| P13 | `R13 PASS` | Formal I-V-only replicate-agreement and interval-contract evidence completed CPU-only. The primary agreement summary is `0.9667128273`; this is a measurement-reliability reference, not model validation, external validation, or an upper bound. A separate manuscript-facing attempt that encountered chromosome-X rows remains recorded as invalid and produced no formal result. Evidence: `alphagenome_custom/metadata/v2/audits/P13/review.json`, `results/v2_p13_iv_interval_contract/`, and `results/v2_p13_replicate_agreement_iv/`. |
| P14 | `R14 PASS` | All 15 hash-verified P12 `B_paper` checkpoints were evaluated on the immutable I-V held-out biological-unit contract. The primary mean was `0.6244769861`; the result is a within-collection reference and does not establish external or unseen-condition generalization. Evidence: `alphagenome_custom/metadata/v2/audits/P14/review.json` and `results/v2_p14_iv_model_reference/`. |
| P15 | `R15 PASS` | The I-V training-interval contract was audited as metadata only: five folds and 600 registered interval rows, with no model, checkpoint, external, chromosome-X, or locked-test signal read. Evidence: `alphagenome_custom/metadata/v2/audits/P15/review.json` and `alphagenome_custom/metadata/v2/p15_iv_training_contract_execution.json`. |
| P16 | `R16 PASS` | Fold-specific I-V training normalization completed for all 241 tracks. The resulting means are recorded in `results/v2_p16_iv_training_normalization/`; no GPU, DNA, checkpoint, external, chromosome-X, or locked-test read was involved. Evidence: `alphagenome_custom/metadata/v2/audits/P16/review.json`. |
| P17 | `R17 PASS` | The 45-job I-V controlled matrix completed on physical GPUs 2/3: `B_iv_dual`, `B_128bp_only`, and `Basenji2_style`, each across five folds and three seeds. The registered matrix prohibited chromosome-X and locked-test access. Evidence: `alphagenome_custom/metadata/v2/audits/P17/review.json` and `results/v2_p17_iv_controlled_matrix/`. |
| P18 | `R18 PASS` | Four external RNA source tracks were reprocessed and audited for the represented-head continuation. P18 stopped at source preparation; it performed no model inference and made no unseen-condition performance claim. Chromosome-X and locked-test access were prohibited. Evidence: `alphagenome_custom/metadata/v2/audits/P18/review.json` and `alphagenome_custom/metadata/v2/p18_external_continuation_execution.json`. |
| P19 | `R19 PASS` | The 60-job represented-head scoring matrix completed on physical GPUs 2/3: four external sources x five folds x three seeds. This is a represented-head external measurement only, not a claim of unseen-model or generalization performance. The failed base-Python attempt is retained separately and excluded. Evidence: `alphagenome_custom/metadata/v2/audits/P19/review.json` and `results/v2_p19_external_scoring/`. |
| P20 | `R20 PASS` | Public eQTL source acquisition and audit completed for three sources, producing 51 I-V candidate variants with zero WBcel235 REF mismatches. This phase performed source and candidate preparation only; it is not a causal or model-performance result. Evidence: `alphagenome_custom/metadata/v2/audits/P20/review.json` and `results/v2_p20_eqtl_variant_set/`. |
| P21 | `R21 PASS` | The 51-candidate eQTL scoring contract completed all 15 registered jobs. Chromosome-X and locked-test access were prohibited. Evidence: `alphagenome_custom/metadata/v2/audits/P21/review.json` and `alphagenome_custom/metadata/v2/p21_eqtl_variant_scoring_execution.json`. |
| P22 | `R22 PASS` | The eQTL effect-association manifest contains 51 registered candidates, 50 analyzable candidates, and 609 samples. Observed-versus-model association was Pearson `-0.007`, Spearman `0.062`, and sign concordance `0.545`; these are descriptive associations, not causal or population-generalization evidence. Evidence: `alphagenome_custom/metadata/v2/audits/P22/review.json` and `results/v2_p22_eqtl_effect_association/`. |
| P23 | `R23 PASS` | The three-configuration LoRA sensitivity pilot completed on one fold and seed `20260714` using physical GPUs 2/3. It is descriptive sensitivity evidence only and cannot replace the formal selection or final-test result. Evidence: `alphagenome_custom/metadata/v2/audits/P23/review.json` and `results/v2_p23_lora_sensitivity_pilot/`. |
| P24 | `R24 PASS` | Borzoi source and adapter feasibility audit completed. The official Borzoi commit and Apache-2.0 license were recorded, but `adapter_status=not_yet_run`; no native human/mouse checkpoint was treated as an eligible C. elegans result, and chromosome-X/locked-test reads were zero. Evidence: `alphagenome_custom/metadata/v2/audits/P24/review.json` and `alphagenome_custom/metadata/v2/p24_borzoi_adapter_execution.json`. |
| P25 | `FAIL before review` | The controller entered the registered Borzoi adapter pilot but stopped before producing a valid execution artifact or R25 review. The Borzoi restore failed with `Layer count mismatch when loading weights from file. Model expected 65 layers, found 66 saved layers.` Therefore P25 has no scientific result and must not be reported as a completed pilot. Evidence: `alphagenome_custom/metadata/v2/p25_borzoi_adapter_pilot_spec.json`, `logs/supervise_p23_p25_borzoi.log`, and controller state. |

Controller reconciliation at the time of this entry: `current_phase=P25`, `status=FAIL`, `updated_at=2026-08-23T10:03:03+00:00`. The truthful release state is therefore P11-P24 reviewed and passing, with P25 a failed pre-result attempt. Preserved failed or invalid attempts include the P12 first attempt, the P13 chromosome-X boundary-violation attempt, and the P19 base-Python attempt; none is silently promoted to evidence.

P10 implementation boundary: the frozen `implementation_sha256` manifest and all completed P10 artifacts remain unchanged. Current worktree drift is reported by historical preflight and review, while controller and worker execution still require an exact current-hash match and fail closed; no historical P10 audit was regenerated.
