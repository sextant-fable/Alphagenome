...
\newcommand{\ExtBenchmarkDataset}{\texttt{[EXT\_BENCHMARK\_DATASET\_ID]}}
\newcommand{\ExtIsolation}{\texttt{[EXT\_SOURCE\_ISOLATION\_RULE]}}
\newcommand{\ExtCondition}{\texttt{[EXT\_UNSEEN\_CONDITION]}}
\newcommand{\ExtCoordinate}{\texttt{[EXT\_REFERENCE\_COORDINATE\_RULE]}}
\newcommand{\ExtLeakage}{\texttt{[EXT\_LEAKAGE\_AUDIT\_RESULT]}}
\newcommand{\ExtProtocol}{\texttt{[EXT\_PROTOCOL\_VERSION]}}
\newcommand{\ExtPrimary}{\texttt{[EXT\_PRIMARY\_EFFECT\_AND\_CI]}}
\newcommand{\ExtBaselines}{\texttt{[EXT\_PUBLIC\_BASELINE\_RESULTS]}}
\newcommand{\ExtStrata}{\texttt{[EXT\_STRATIFIED\_RESULTS]}}
\newcommand{\ExtVariantDataset}{\texttt{[EXT\_VARIANT\_DATASET\_ID]}}
\newcommand{\ExtVariant}{\texttt{[EXT\_VARIANT\_RESULT]}}
\newcommand{\ExtRelease}{\texttt{[EXT\_RELEASE\_MANIFEST]}}
\newcommand{\draftfigure}[2]{%
  \begin{figure*}[t]
    \centering
    \fbox{\begin{minipage}{0.92\textwidth}
      \centering\vspace{8mm}
      {\color{draftblue}\Large Draft figure placeholder}\par
      \vspace{2mm}\small The source-data table and plotting script will be
      attached after the corresponding frozen artifact is available.\vspace{8mm}
    \end{minipage}}
    \caption{#2}\label{#1}
  \end{figure*}}
...
\author*[1]{\fnm{Author} \sur{Name}}\email{\texttt{AUTHOR\_EMAIL\_TO\_BE\_SUPPLIED}}
\affil*[1]{\orgdiv{\texttt{AUTHOR\_DEPARTMENT\_TO\_BE\_SUPPLIED}}, \orgname{\texttt{AUTHOR\_INSTITUTION\_TO\_BE\_SUPPLIED}}, \orgaddress{\city{\texttt{AUTHOR\_CITY\_TO\_BE\_SUPPLIED}}, \country{\texttt{AUTHOR\_COUNTRY\_TO\_BE\_SUPPLIED}}}}
...
\midrule
Full Model B & \ExtCoordinate; 131,072 bp & 1-bp and 128-bp RNA coverage; \ExtProtocol & To be populated & Primary method; \ExtPrimary \\
Model A & Same & Same & To be populated & Internal transfer baseline \\
Size-matched Model C & Same & Same & Matched trainable budget & From-scratch control \\
Enformer & Adapter-dependent & Shared protocol only if RNA adapter is faithful & To be populated & \ExtBaselines \\
Borzoi & Adapter-dependent & Shared protocol only if RNA adapter is faithful & To be populated & \ExtBaselines \\
Other public methods & Method-specific & Excluded when not technically comparable & To be populated & Reason recorded explicitly \\
\bottomrule
\end{tabular}
\end{table*}
...
manifest remains an \texttt{AUTHOR\_INPUT\_NEEDED} item rather than an invented value.
...
under \ExtRelease. Repository, accession, license and embargo fields are
\texttt{AUTHOR\_INPUT\_NEEDED} until the release manifest is signed.
...
commands used for the reported figures. The immutable code identifier is
\texttt{AUTHOR\_INPUT\_NEEDED} until the release is tagged.

\section*{Acknowledgements}

\texttt{AUTHOR\_INPUT\_NEEDED}.
